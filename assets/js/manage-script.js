mailster = (function (mailster, $, window, document) {
	...
	typeof wpUploaderInit == 'object' &&
		mailster.events.push('documentReady', uploader_init);
	...
	$('#import_wordpress').on('submit', function () {
						...
						},
						{
							...
							complete: function () {
								...
								if (finished) {
									...
									progress.addClass('finished');
									$('.step2-body').html(
										mailster.l10n.manage.export_finished
									);
					...
					progressbar.animate(
						...
						},
						{
							...
							step: function (percentage) {
								importstatus.html(
									mailster.util.sprintf(
										mailster.l10n.manage.import_contacts,
										Math.ceil(percentage) + '%'
									)
								);
							},
			...
			errorint = setInterval(function () {
				if (i <= 0) {
					...
					str =
						'<span class="error">' +
						mailster.util.sprintf(
							mailster.l10n.manage.continues_in,
							i--
						) +
						'</span>';
				}
