mailster = (function (mailster, $, window, document) {
	...
	mailster.$.window
		.on('click', '.upload-addon', function () {
			$('.upload-field').show();
		});
	...
	var overlay = function () {
		...
		var open = function (addon) {
				...
				overlay.find('.theme-author').html(mailster.util.sprintf('By ' + data.author));
			...
			remove = function () {
				...
				if ('index.html' == file && confirm(mailster.util.sprintf(mailster.l10n.addons.confirm_delete, '"' + data.name + '"'))) {
					deleteTemplate(data.slug);
					close();
				} else if (confirm(mailster.util.sprintf(mailster.l10n.addons.confirm_delete_file, '"' + file + '"', '"' + data.name + '"'))) {
			...
			makedefault = function () {
				if (confirm(mailster.util.sprintf(mailster.l10n.addons.confirm_default, '"' + data.name + '"'))) {

					var addon = $('[data-slug="' + data.slug + '"]');

					busy = true;

					mailster.util.ajax('default_addon', {
						slug: data.slug,
					}, function (response) {

						if (response.success) {
							setFilter('installed', function () {
								$('[data-slug="' + data.slug + '"]').find('.notice-success').html('<p>' + response.msg + '</p>');
							});
						...
						addon.removeClass('loading').find('.updating-message').removeClass('updating-message');
						busy = false;
						close();
			...
			campaign = function () {
				...
				document.location = this.href + '&addon=' + data.slug + '&file=' + overlay.find('.theme-file-selector').val();
	...
	function installAddon(slug) {
		...
		addon.find('.notice-warning').addClass('updating-message').html('<p>' + mailster.l10n.addons.installing + '</p>');
	...
	function installAddonFromUrl(url, slug) {
		...
		addon.addClass('loading').find('.request-download').addClass('updating-message');
		addon.find('.notice-warning').addClass('updating-message').html('<p>' + mailster.l10n.addons.installing + '</p>');
		return;
		mailster.util.ajax('quick_install', {
			url: url,
			...
			if (response.success) {
				addon.addClass('is-installed').find('.notice-warning').removeClass('updating-message notice-warning').addClass('notice-success').html('<p>' + mailster.l10n.addons.installed + '</p>');
				addon.find('.notice-error').empty();
			} else {
				addon.find('.notice-error').html('<p>' + response.msg + '</p>');
			}
	...
	function activateAddon(slug) {
		...
		addon.addClass('loading');
		addon.find('.notice-warning').addClass('updating-message').html('<p>1' + mailster.l10n.addons.activating + '</p>');
	...
	function deactivateAddon(slug) {
		...
		addon.addClass('loading');
		addon.find('.notice-warning').addClass('updating-message').html('<p>2' + mailster.l10n.addons.deactivating + '</p>');
	...
	function updateTemplateFromUrl(url, slug) {
		...
		addon.addClass('loading').find('.request-download').addClass('updating-message');
		addon.find('.notice-warning').addClass('updating-message').html('<p>' + mailster.l10n.addons.updating + '</p>');
		mailster.util.ajax('download_addon', {
			url: url,
			slug: slug,
		}, function (response) {

			if (response.success) {
				addon.find('.notice-warning').removeClass('updating-message notice-warning').addClass('notice-success').html('<p>' + mailster.l10n.addons.updated + '</p>');
				addon.find('.notice-error').empty();
	...
	function deleteTemplate(slug, file, cb) {
		...
		mailster.util.ajax('delete_addon', {
			slug: slug,
			file: file || null,
		}, function (response) {
	...
	function errorTemplate(slug, errormsg) {
		...
		addon.find('.notice-error').html('<p>' + $('<div>' + errormsg + '</div>').text() + '</p>');
	...
	function query(cb) {
		...
		mailster.util.ajax('query_addons', {
