mailster = (function (mailster, $, window, document) {
	...
	mailster.$.document
		.on('change', '#export-subscribers input,#export-subscribers select', update_export_count)
		...
		mailster.util.ajax('do_export', {
			...
			if (response.success) {
				...
				if (finished) {
					...
					exportstatus.html(mailster.util.sprintf(mailster.l10n.manage.write_file, response.total, Math.ceil(percentage) + '%'));
				}
	...
	function update_export_count() {
		...
		setTimeout(function () {
			...
			mailster.util.ajax('get_subscriber_count', {
				...
				if (response.success) {
					$('#export-subscriber-button').val(mailster.util.sprintf(mailster.l10n.manage.export_n_subscribers, response.count_formated)).prop('disabled', !response.count);
				}
