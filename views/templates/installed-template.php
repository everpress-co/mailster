...
?>
	<li class="<?php echo implode( ' ', $class ); ?>" id="template-<?php echo esc_attr( $slug ); ?>" name="mailster_template_<?php echo $i; ?>" data-id="<?php echo $i++; ?>">
		...
		<div class="meta">
			<h3><?php echo esc_html( $data['name'] ); ?> <span class="version"><?php echo esc_html( $data['version'] ); ?></span>
				<?php if ( $update ) : ?>
					<?php if ( ! $is_free && $envato_item_id ) : ?>
							...
							<a title="<?php esc_html_e( 'update via Envato', 'mailster' ); ?>" class="update envato-activate button button-primary button-small alignright" href="
														<?php
														echo add_query_arg(
															array(
														...
														?>
																																													" data-slug="<?php echo $slug; ?>"><?php echo sprintf( esc_html__( 'Update to %s', 'mailster' ), $mailster_templates[ $slug ]['new_version'] ); ?></a>
			...
			<div>
				<?php esc_html_e( 'by', 'mailster' ); ?> <?php
	...
	?>
						<?php echo esc_html( $data['author'] ); ?><?php endif; ?>
			</div>
		</div>
		...
		<div class="action-links">
			<ul>
				...
				<?php endif; ?>
				<?php
				if ( current_user_can( 'mailster_edit_templates' ) ) :
					$writeable = wp_is_writable( $t->path . '/' . $slug . '/index.html' );
					?>

					<li><a title="<?php echo esc_attr( sprintf( 'Edit %s', '"' . $data['name'] . '"' ) ); ?>" class="edit <?php echo ! $writeable ? 'disabled' : ''; ?> button" data-slug="<?php echo esc_attr( $slug ); ?>" href="<?php echo $slug . '/index.html'; ?>" 
											 <?php
												if ( ! $writeable ) :
													?>
