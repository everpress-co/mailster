...
?>
	...
	<?php if ( ! mailster()->is( 'setup' ) && $geoip && is_file( $geo_db_file_countries ) ) : ?>
	<tr valign="top">
		<th scope="row"></th>
		<td>
	...
	<?php endif; ?>
		...
		<strong><?php esc_html_e( 'Your country', 'mailster' ) ?>:</strong> <?php echo mailster_ip2Country( '', 'name' ) ?><br>&nbsp;&nbsp;<strong><?php esc_html_e( 'Last update', 'mailster' ) ?>: <?php echo date( $timeformat, filemtime( $geo_db_file_countries ) + $timeoffset ) ?> </strong><br>
	<?php if ( is_file( $geo_db_file_cities ) ) : ?>
		<strong><?php esc_html_e( 'Your city', 'mailster' ) ?>:</strong> <?php echo mailster_ip2City( '', 'city' ) ?><br>&nbsp;&nbsp;<strong><?php esc_html_e( 'Last update', 'mailster' ) ?>: <?php echo date( $timeformat, filemtime( $geo_db_file_cities ) + $timeoffset ) ?></strong>
	<?php endif; ?>
		</p>
		<?php if ( $geoip ) : ?>
		...
		<?php endif; ?>
		<p class="description">This product includes GeoLite data created by MaxMind, available from <a href="https://www.maxmind.com" class="external">maxmind.com</a></p>
		</td>
	</tr>
	<?php
	endif;
