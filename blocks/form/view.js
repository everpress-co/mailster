(function () {
	...
	// initialize instant or wait of DOMis not loaded yet
	...
	function app() {
		...
		Array.prototype.forEach.call(forms, function (form, i) {
			...
			form.addEventListener('submit', function (event) {
				...
				function handlerResponse(response) {
					...
					if (
						true ||
						scrollPosition < form.getBoundingClientRect().top
					) {
						form.insertBefore(
							info,
		...
		function openForm(form, placement, explicit) {
			if (explicit || !hasBeendShown(placement)) {
				var wrap = form.closest(
						'.wp-block-mailster-form-outside-wrapper'
					),
					closeButton = wrap.querySelector(
		...
		function closeForm(form, placement, explicit) {
			...
			document
				.querySelector('body')
				.classList.remove('mailster-form-active');
		}
